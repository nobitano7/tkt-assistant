...
import { GoogleGenAI, Chat, FunctionDeclaration, Type, Part, Content } from "@google/genai";
import { type ItineraryGroup, type BookingInfo, type AirportInfo } from '../types';
...
export async function parseBookingToMessages(content: string | Part[]): Promise<BookingInfo> {
  if (!process.env.API_KEY) {
    throw new Error("API_KEY is not set in environment variables.");
  }
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
...
`;
  ...
  const response = await ai.models.generateContent({
    model: 'gemini-2.5-flash',
    contents: typeof content === 'string' ? { parts: [{ text: prompt }, { text: content }]} : { parts: [{ text: prompt }, ...content] },
    config: {
      responseMimeType: "application/json",
      responseSchema: {
        type: Type.OBJECT,
  ...
  const jsonString = response.text.trim();
  return JSON.parse(jsonString) as BookingInfo;
}
...
export async function parseGroupFareRequest(content: string | Part[]): Promise<GroupFareFlightInfo[]> {
  if (!process.env.API_KEY) {
    throw new Error("API_KEY is not set in environment variables.");
  }
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
...
`;
  ...
  const jsonString = response.text.trim();
  return JSON.parse(jsonString) as GroupFareFlightInfo[];
}


// Helper function to simulate calling the TIMATIC tool.
export async function runTimaticTool(nationality: string, destination: string, transitPoints?: string[], suggestAlternatives?: boolean): Promise<string> {
    if (!process.env.API_KEY) {
      throw new Error("API_KEY is not set in environment variables.");
    }
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
...
// Helper function to run the SR DOCS generation tool.
export function runGenerateSrDocsTool(args: any): { command: string } {
    const {
        issuingCountryCode,
        passportNumber,
        nationalityCode,
...
export async function findNearestAirports(location: string): Promise<AirportInfo[]> {
  ...
  const jsonString = response.text.trim();
  return JSON.parse(jsonString) as AirportInfo[];
}

export async function extractTimaticDetailsFromBooking(bookingText: string): Promise<{ nationality: string; destination: string; transitPoints: string[] }> {
    if (!process.env.API_KEY) {
      throw new Error("API_KEY is not set in environment variables.");
    }
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        ...
        config: {
            ...
            responseSchema: {
                ...
                properties: {
                    nationality: {
                        type: Type.STRING,
                        description: "The passenger's nationality as a country name (e.g., 'Việt Nam')."
                    },
                    destination: {
                        type: Type.STRING,
                        description: "The final destination as a country name (e.g., 'Mỹ')."
                    },
                    transitPoints: {
                        type: Type.ARRAY,
                        description: "An array of transit countries as country names.",
                        items: {
                            type: Type.STRING
                        }
                    }
...
export async function runGdsEncoderTool(tool: string, params: any): Promise<{ result: string }> {
    ...
    switch (tool) {
        ...
        case 'currency_conversion':
            ...
            break;
        default:
            throw new Error('Invalid GDS tool specified.');
    }
