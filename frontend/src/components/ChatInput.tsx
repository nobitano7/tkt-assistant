...
import { type DocumentWithContent } from '../hooks/useDocuments';
...
interface ChatInputProps {
  ...
  contextualDocument: DocumentWithContent | null;
  onClearContext: () => void;
}

export const ChatInput: React.FC<ChatInputProps> = ({ input, setInput, onSendMessage, isLoading, attachedImage, setAttachedImage, contextualDocument, onClearContext }) => {
  ...
  return (
    <footer className="bg-white p-4 border-t border-slate-200">
      {contextualDocument && (
        <div className="p-2 mb-2 bg-sky-50 border border-sky-200 rounded-lg flex items-center justify-between text-sm animate-fade-in">
          <div className="flex items-center space-x-2 text-sky-700 overflow-hidden">
            ...
            <span className="truncate" title={contextualDocument.name}>{contextualDocument.name}</span>
          </div>
          <button onClick={onClearContext} className="p-1 rounded-full text-slate-500 hover:bg-slate-200" aria-label="Clear context">
      ...
      <div className="relative">
        ...
        <button
          ...
          disabled={isLoading || (!input.trim() && !attachedImage && !contextualDocument)}
