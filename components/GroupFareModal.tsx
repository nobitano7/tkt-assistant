...
export const GroupFareModal: React.FC<GroupFareModalProps> = ({ isOpen, onClose }) => {
    ...
    useEffect(() => {
        // When tab switches to VJ, apply fixed agent info
        if (activeTab === 'VJ') {
            ...
            // For existing data in the results table
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            if (attachedFile) {
                ...
                content = inputText.trim() ? [{ text: inputText }, filePart] : [filePart];
            } else {
                content = inputText;
            }
            const parsedData = await parseGroupFareRequest(content);
             // If user analyzes while on VJ tab, immediately apply fixed info
            if (activeTab === 'VJ') {
    ...
    const handlePaste = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
        ...
        for (let i = 0; i < items.length; i++) {
            if (items[i].type.startsWith('image/')) {
                ...
                if (file) {
                    setAttachedFile(file);
                    e.preventDefault(); // Prevent pasting base64 string into textarea
                    return;
                }
            }
        }
    ...
    const handleParseAndAdd = async () => {
        ...
        try {
            ...
            if (attachedFile) {
                const filePart = await fileToGenerativePart(attachedFile);
                content = manualParseText.trim() ? [{ text: manualParseText }, filePart] : [filePart];
            } else {
                content = manualParseText;
            }
            const parsedData = await parseGroupFareRequest(content);
                ...
                <main className="overflow-y-auto flex-1 bg-slate-50">
                     <div className="px-6 pt-4 border-b border-slate-200 bg-slate-50 sticky top-0 z-10">
                        <nav className="flex space-x-1">
                            <TabButton isActive={activeTab === 'parser'} onClick={() => setActiveTab('parser')}>Phân tích AI</TabButton>
                            <TabButton isActive={activeTab === 'VNA'} onClick={() => setActiveTab('VNA')}>VNA</TabButton>
                            <TabButton isActive={activeTab === 'VJ'} onClick={() => setActiveTab('VJ')}>VJ</TabButton>
                            <TabButton isActive={activeTab === 'QH'} onClick={() => setActiveTab('QH')}>QH</TabButton>
                            <TabButton isActive={activeTab === 'VU'} onClick={() => setActiveTab('VU')}>VU</TabButton>
                        </nav>
                    </div>
                                ...
                                <div className="pt-4 border-t">
                                    ...
                                    <div className="flex justify-between items-center mt-2">
                                        <button onClick={() => fileInputRef.current?.click()} disabled={isParsing} className="p-2 rounded-lg border border-slate-300 bg-white text-slate-600 hover:bg-slate-100 disabled:opacity-50 transition-colors flex items-center space-x-2 text-sm">
                                         ...
                                         <button onClick={handleParseAndAdd} disabled={isParsing || (!manualParseText && !attachedFile)} className="px-3 py-2 text-sm rounded-lg bg-sky-600 text-white font-bold hover:bg-sky-500 transition-all flex items-center space-x-2 disabled:bg-slate-300">
                     ...
                     <div className="flex items-center space-x-3">
                         {parsedFlights.length > 0 && (
                            <div className="flex items-center space-x-3 animate-fade-in">
                                ...
                                <button onClick={handleExportExcel} className="px-3 py-2 rounded-lg bg-green-600 text-white font-bold hover:bg-green-500 transition-all flex items-center space-x-2">
                         ...
                         {activeTab === 'parser' && (
                             <button onClick={handleAnalyze} disabled={isParsing || (!inputText && !attachedFile)} className="px-4 py-2 rounded-lg bg-sky-600 text-white font-bold hover:bg-sky-500 disabled:bg-slate-300 transition-all flex items-center space-x-2 w-32 justify-center">
                                {isParsing ? (
