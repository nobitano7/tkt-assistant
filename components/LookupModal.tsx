...
import { runTimaticTool, extractTimaticDetailsFromBooking, runGdsEncoderTool } from '../services/geminiService';
    ...
    const handleSearch = async () => {
        ...
        try {
             if (bookingText.trim()) {
                ...
                const transitArray = extractedDetails.transitPoints || [];
                setTransitPoints(transitArray.join(', '));
                
                const timaticResult = await runTimaticTool(extractedDetails.nationality, extractedDetails.destination, transitArray);
